document.addEventListener("DOMContentLoaded", () => {
    ...
    const elegirFiltro = document.getElementById("filtroTareas");
    ...
    function muestraTareas() {
        let contenidoHTML = "";
        const filtro = elegirFiltro.value;
        ...
        for (let i = 0; i < tareasFiltradas.length; i++) {
            ...
            const estiloTexto = tarea.completada ? 'style="text-decoration: line-through;"' : '';
            const tareasActuales = tareas.indexOf(tarea); 

            contenidoHTML += `
                <tr>
                    <td>${i + 1}</td>
                    <td ${estiloTexto}>${tarea.nombre}</td>
                    <td><input type="checkbox" ${tarea.completada ? 'checked' : ''} onchange="alternarTarea(${tareasActuales})"></td>
                    <td><button onclick="eliminarTarea(${tareasActuales})" class="btn btn-danger">X</button></td>
                </tr>
            `;
        }
    ...
    elegirFiltro.addEventListener("change", muestraTareas);
